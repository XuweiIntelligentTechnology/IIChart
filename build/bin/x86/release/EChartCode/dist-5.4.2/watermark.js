...
const watermark = function () {
    return {
        build: function(arg) {
            ...
            const setting = {
                text: "Shenzhen Xuwei Intelligent@Copyright",    // 默认水印内容
                beginX: 0,              // 默认起始x坐标
                endX: 0,                 // 默认结束x坐标
                beginY: 50,              // 默认起始y坐标
                endY: 0,                 // 默认结束y坐标
			...
			if ((maxWidth - setting.beginX - setting.endX) - cols*(setting.width + setting.intervalX) >= setting.width + setting.beginX)
			{
				cols = cols + 1;
			}
            // 计算行个数
            let rows = parseInt((maxHeight - setting.beginY - setting.endY) / (setting.height + setting.intervalY));
			if ((maxHeight - setting.beginY - setting.endY) - rows*(setting.height + setting.intervalY) >= setting.height + setting.beginY)
			{
				rows = rows + 1;
			}
