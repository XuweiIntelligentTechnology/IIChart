#pragma once
/***********************************************************************
 * Module:   ISunburstTypeItemStyle.h
 * Author:   杨鹏辉
 * Modified: 2022年7月1日 15:02:44
 * Purpose:  旭日图扇形块的样式。
 *           可以在 series.itemStyle 定义所有扇形块的样式，也可以在 series.levels.itemStyle 定义每一层扇形块的样式，
 *            还可以在 series.data.itemStyle 定义每个扇形块单独的样式，这三者的优先级从低到高。也就是说，如果定义
 *			 了 series.data.itemStyle，将会覆盖 series.itemStyle 和 series.levels.itemStyle。
...
namespace IIChart
{
	namespace Sunburst
	{
		/*@class
		*************************************************************************
		* 类  名: ISunburstTypeItemStyle
		* 修  改: 2022年7月1日 15:02:44
		* 功  能: 旭日图扇形块的样式。
		-----------------------------------------------------------------------
		作  者:  YangPenghui 2022-10-26;
		************************************************************************/
		class ISunburstTypeItemStyle
		{
		public:
			...
			******************************************************************
			 功  能：用于指定扇形区块的内外圆角半径
					 左上，右上，左下，右下都用一样的圆角;
			 参  数：<nValue> 内圆角半径和外圆角半径;
					 <bIsPercentage> 为 false 时，nValue 为数值单位为px,
									 为 true 时，nValue 为相对于扇形区块的半径的百分比值,
			 返回值：<错误码>;
			 ------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual int  SetBorderRadius(int nValue, bool bIsPercentage = false) = 0;
			virtual void  GetBorderRadius(int* pnValue) = 0;

			/*@function
			******************************************************************
			 功  能：用于指定扇形区块的内外圆角半径
			 参  数：<nValue1> 表示为环形图时，内圆角半径;
					 <nValue2> 表示为环形图时，外圆角半径;
					 <bIsPercentage> 为 false 时，nValue1、nValue2 为数值单位为px,
									 为 true 时，nValue1、nValue2 为相对于扇形区块的半径的百分比值,
			 返回值：<错误码>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual int  SetBorderRadius(int nValue1, int nValue2, bool bIsPercentage = false) = 0;
			...
			******************************************************************
			 ...
			 参  数：<nValue1> 左上的圆角;
					 ...
					 <nValue3> 右下的圆角;
					 <nValue4> 左下的圆角;
					 <bIsPercentage> 为 false 时，nValue1、nValue2、nValue3、nValue4 为数值单位为px,
									 为 true 时，nValue1、nValue2、nValue3、nValue4 为相对于扇形区块的半径的百分比值,
			 返回值：<错误码>;
			 ------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual int SetBorderRadius(int nValue1, int nValue2, int nValue3, int nValue4, bool bIsPercentage = false) = 0;
