#pragma once
/***********************************************************************
 * Module:  ILineTypeData.h
 * Author:  杨鹏辉
 * Modified: 2022年7月1日 15:02:44
 * Purpose: 系列中的数据内容数组。数组项通常为具体的数据项。
 *          注意：
 *          1. 如果系列没有指定 data，并且 option 有 dataset，那么默认使用第一个 dataset。
 *          2. 如果指定了 data，则不会再使用 dataset。
 *          3. 可以使用 series.datasetIndex 指定其他的 dataset。
 * 
 *          一、通常来说，数据用一个二维数组表示。如下，每一列被称为一个『维度』。
 *			 series: [{
 *				data: [
 *					// 维度X   维度Y   其他维度 ...
 *					[  3.4,    4.5,   15,   43],
 ...
 *                >>> 其他的时间字符串，包括（均表示本地时间）: '2012', '2012-3-1', '2012/3/1', '2012/03/01', '2009/6/12 2:00',
 *                    '2009/6/12 2:05:08', '2009/6/12 2:05:08.123'
 *             >> 或者用户自行初始化的 Date 实例：
 *                注意，用户自行初始化 Date 实例的时候，浏览器的行为有差异，不同字符串的表示也不同。
 *                例如：在 chrome 中，new Date('2012-01-01') 表示 UTC 时间的 2012 年 1 月 1 日，而 new Date('2012-1-1') 和 new
 *                    Date('2012/01/01') 表示本地时间的 2012 年 1 月 1 日。在 safari 中，不支持 new Date('2012-1-1') 这种表示方法。
...
#include "Options/GeneralOptions/Series/Line/ILineTypeDataLabelLine.h"
...
#include "Options/GeneralOptions/Series/Line/ILineTypeDataSelect.h"
		  
#include "Options/GeneralOptions/Series/Line/ILineTypeDataBlur.h"
#include "Options/GeneralOptions/Series/Line/ILineTypeDataEmphasis.h"
...
namespace IIChart
{
	namespace Line
	{
		...
		************************************************************************/
		class ILineTypeData : virtual public IBaseArrayElement
		{
		public:
			...
			*******************************************************************
			功  能: 标签的视觉引导线配置
			参  数: <无>;
			返回值: <ILineTypeDataLabelLine*>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual ILineTypeDataLabelLine* GetLabelLine() = 0;
			...
			*******************************************************************
			功  能: 单个拐点的高亮样式和标签设置
			参  数: <无>;
			返回值: <ILineTypeDataEmphasis*>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual ILineTypeDataEmphasis* GetEmphasis() = 0;

			/*@function
			*******************************************************************
			功  能: 单个拐点的淡出样式设置。
			参  数: <无>;
			返回值: <ILineTypeDataBlur*>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual ILineTypeDataBlur* GetBlur() = 0;

			/*@function
			*******************************************************************
			功  能: 单个拐点的选中样式和标签设置。
			参  数: <无>;
			返回值: <ILineTypeDataSelect*>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual ILineTypeDataSelect* GetSelect() = 0;
