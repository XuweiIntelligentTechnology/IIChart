#pragma once
/***********************************************************************
 * Module:  IPieTypeLabel.h
 * Author:  杨鹏辉
 * Modified: 2022年7月1日 15:02:44
 * Purpose: 饼图图形上的标签，可用于说明图形的一些数据信息，比如值，名称等。
 ***********************************************************************/
...
namespace IIChart
{
	namespace Pie
	{
		...
		* 功  能: 图形上的标签，可用于说明图形的一些数据信息，比如值，名称等。
		-----------------------------------------------------------------------
		...
		************************************************************************/
		...
		public:
			...
			******************************************************************/
			...
			/*@function
			******************************************************************
			功  能：标签的对齐方式，仅当 position 值为 'outer' 时有效
			参  数：<eValue>;
			返回值：<无>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual void SetAlignTo(EAlignTo eValue) = 0;
			virtual EAlignTo GetAlignTo() const = 0;

			/*@function
			******************************************************************
			功  能：文字边距，仅当 label.position 为 'outer' 并且 label.alignTo 为 'edge' 时有效。
			参  数：<nValue>;
			        <bIsPercent>;
			返回值：<无>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual void SetEdgeDistance(int nValue, bool bIsPercent) = 0;
			virtual void GetEdgeDistance(int* pnValue, bool* pbIsPercent) const = 0;

			/*@function
			******************************************************************
			功  能：文字的出血线大小，超过出血线的文字将被裁剪为 '...'。
			注  意：仅当 label.position 为 'outer' 并且 label.alignTo 为 'none' 或 'labelLine' 的情况有效。
			参  数：<nValue>;
			返回值：<无>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual void SetBleedMargin(int nValue) = 0;
			virtual int  GetBleedMargin() const = 0;

			/*@function
			******************************************************************
			功  能：文字与 label line 之间的距离。
			参  数：<nValue>;
			返回值：<无>;
			------------------------------------------------------------------
			作  者:  YangPenghui 2022-10-26;
			******************************************************************/
			virtual void SetDistanceToLabelLine(int nValue) = 0;
			virtual int  GetDistanceToLabelLine() const = 0;
		};
	}
}
